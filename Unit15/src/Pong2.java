...
public class Pong2 extends Canvas implements KeyListener, Runnable
{
	...
	public Pong2()
	{
		...
		for (int i = 5; i < 750; i+=85) {
			for (int j = 5; j < 90; j+=45) {
				tiles.add(new Tile(i, j, 100, 50, Color.GREEN));
			}
		}
		...
		for (int i = 5; i < 750; i+=85) {
			for (int j = 680; j < 750; j+=45) {
				tiles.add(new Tile(i, j, 100, 50, Color.YELLOW));
			}
		}
		...
		for (int j = 95; j < 660; j+=45) {
			for (int i = 5; i < 175; i+=85) {
				tiles.add(new Tile(i, j, 100, 50, Color.MAGENTA));
			}
		}
		...
		for (int j = 95; j < 660; j+=45) {
			for (int i = 600; i < 750; i+=85) {
				tiles.add(new Tile(i, j, 100, 50, Color.BLUE));
			}
		}
   ...
   public void paint(Graphics window)
   {
		...
		if (tiles.size() == 0) {
			...
			for (int i = 5; i < 750; i += 85) {
				for (int j = 680; j < 750; j += 45) {
					tiles.add(new Tile(i, j, 100, 50, Color.DARK_GRAY));
				}
			}
			...
			for (int j = 95; j < 660; j += 45) {
				for (int i = 5; i < 175; i += 85) {
					tiles.add(new Tile(i, j, 100, 50, Color.BLACK));
				}
			}
			...
			for (int j = 95; j < 660; j += 45) {
				for (int i = 600; i < 750; i += 85) {
					tiles.add(new Tile(i, j, 100, 50, Color.LIGHT_GRAY));
				}
			}
		...
		for (int i = 0; i < tiles.size(); i++) {
			if (lCollide(tiles.get(i)) || rCollide(tiles.get(i))) {
				tiles.get(i).draw(graphToBack, Color.WHITE);
				tiles.remove(i);
				ball.setXSpeed(-ball.getXSpeed());
			}
			if (tCollide(tiles.get(i)) || bCollide(tiles.get(i))) {
				tiles.get(i).draw(graphToBack, Color.WHITE);
				tiles.remove(i);
				ball.setYSpeed(-ball.getYSpeed());
			}
